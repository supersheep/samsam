...
function tests(method, body) {
  ...
  function pass(name) {
    ...
    it("should return true for " + name, function() {
  ...
  function fail(name) {
    var args = Array.prototype.slice.call(arguments, 1);
    it("should return false for " + name, function() {
      assert(!samsam[method].apply(samsam, args));
    });
  }
...
tests("deepEqual", function(pass, fail) {
  ...
  pass("objects with one property", {
    id: 42
  }, {
    id: 42
  });
  ...
  pass("complex objects", deepObject, {
    ...
    child: {
      speaking: deepObject.child.speaking
    },
    id: 42,
    name: "Hey"
  });

  pass("arrays", [1, 2, "Hey there", func, {
    id: 42,
    prop: [2, 3]
  }], [1, 2, "Hey there", func, {
    id: 42,
  ...
  fail("nested array with shallow array", [
    ["hey"]
  ], ["hey"]);
...
tests("match", function(pass, fail, shouldThrow, add) {
  ...
  add("should call matcher with object", function() {
    ...
    assert.deepEqual(_args, ["Assertions 123"]);
  });
  ...
  pass("empty strings", "", "");
  pass("empty strings as object properties", {
    foo: ""
  }, {
    foo: ""
  });
  ...
  pass("matching array subset", [1, 2, 3, {
    id: 42
  }], [{
    id: 42
  }]);
  ...
  pass("empty arrays", [], []);
  pass("objects with empty arrays", {
    xs: []
  }, {
    xs: []
  });
  fail("nested objects with different depth", {
    a: 1
  }, {
    b: {
      c: 2
...
tests("isArguments", function(pass, fail) {
  ...
  fail("primitive", 42);
  fail("object", {});
  pass("arguments object from strict-mode function", (function() {
    "use strict";
    return arguments;
  }()));
});